...
def find_balance_sheet_pages_v2(df):
    ...
    """finds more balance sheet pages, less conservative than the original
       made for checking how useful this is, not actually tested or anything. Use with care."""

    agg_lines = aggregate_sentences_over_lines(df)

    # detect different strings in each line
    ...
    # we include pages that have balance sheet or statment of finacial whatever that don't include
    # notes to the financial statements or statement of changes in equity
...
def process_PDF(filepath, version_2 = False):
    """
    ...
    if version_2:
        print("using version 2 of finding balance sheet - not tested so use with care. Set version_2 = False if unsure")
        csv_numbers = find_balance_sheet_pages_v2(data)
    else:
    ...
    # added try to process pdfs with no balance page identified
    try:
